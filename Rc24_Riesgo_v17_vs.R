...
# series flood raster 
load_flood_raster_list <- function(series_return_period, folder_path) {
  
  raster_depth_files <- list.files(path = folder_path, pattern = "\\.tif$", full.names = TRUE) # raster files
...
list_floodseries <- load_flood_raster_list(series_return_period, "series_flood_raster\\raster_depth_10m") # load raster without project
...
df_villa <- st_sf(
  ...
  crs = "EPSG:32718"  # definir el sistema de referencia
)
...
df_survey <- bind_rows(
  ...
  read.csv("survey/survey_2024.csv",  na.strings = c("", "-")) %>%
    ...
    dplyr::mutate(household_id_survey = folio,
                  ...
                  #household_flood_current_year = ifelse(d19 == "sí" & d20 == "sí", TRUE, FALSE),
                  household_flood_current_year = case_when(
                    d19 == "sí" ~ TRUE,
                    d19 == "no" ~ FALSE,
                    d19 == "no aplica" ~ FALSE,
                    TRUE ~ NA),
                  ...
                  household_measure_flood_proof = !is.na(coalesce(g36_1_1, g36_1_2, g36_1_3, g36_2_1, g36_2_2, g36_2_3)),
  ...
  select(household_id_survey, household_address, year_current, household_year_entry, household_year_existence, 
         household_measure_flood_proof, household_measure, household_flood_current_year, household_flood_once, household_flood_risk_perception) %>%
...
model_reduction_vulnerability <- function(list_household_model_development, household_compliance_threshold) { 
  #list_household_model_development <- list_household_inter; household_compliance_threshold <- 0.5
  
  # add compliance rate
  household_compliance_rate <- list_household_model_development %>%
    map(as_tibble) %>%
    reduce(bind_rows) %>%
    distinct(household_id_model) %>%
    mutate(household_compliance_rate = runif(n(), 0, 1))  # random values for unique id
  
  # bind
  list_household_model_development <- list_household_model_development %>%
    map(as_tibble) %>%
    reduce(bind_rows) %>%
    left_join(household_compliance_rate, by = "household_id_model") %>%  
...
model_reduction_hazard <- function(list_household_model_development, ua_2) { 
  ...
  list_household_model_hazard <- map2(list_household_model_development, seq_along(series_infrastructure), function(household, i) {
    ...
    depth[is.na(depth)] <- 0
    household$flood_return_period <- series_return_period[[i]]
    household$flood_depth <- depth[[1]]
    return(household)
  })
...
# model: risk ----
model_risk <- function(list_household_model_vulnerability_hazard, household_compliance_threshold, f, learning, r) { 
  ...
  for (id_unique in unique(df_household_risk$household_id_model)) {
    ...
    df <- df_household_risk %>%
      ...
      mutate(household_learning_current_year = NA,
             household_learning_accumulated = 0)
    
    for (i in seq_len(nrow(df))) {
      
      df$household_measure_flood_proof[i] = df$household_compliance_rate[i] > household_compliance_threshold    # Define si el hogar se "levanta" o no
      df$household_flood_current_year[i] = case_when(
        df$flood_depth[i]  > f ~ TRUE,
        df$flood_depth[i]  == 0 ~ FALSE,
        df$flood_depth[i]  < f & !df$household_measure_flood_proof[i] ~ TRUE,
        df$flood_depth[i]  < f & df$household_measure_flood_proof[i] ~ FALSE
      )
      ...
      if (i <= nrow(df) - 1) { 
        df$household_compliance_rate[i + 1] = case_when(
          ...
          # learning effect
          df$household_fb[i] == 2 &  df$household_flood_current_year[i] ~ df$household_compliance_rate[i] + learning, 
          df$household_fb[i] == 2 & !df$household_flood_current_year[i] ~ df$household_compliance_rate[i],                        
          # levee effect
          df$household_fb[i] == 3 &  df$household_flood_current_year[i] ~ df$household_compliance_rate[i] + learning,                        
          df$household_fb[i] == 3 & !df$household_flood_current_year[i] ~ df$household_compliance_rate[i] - learning/r,  
          # good students
          df$household_fb[i] == 4 &  df$household_flood_current_year[i] ~ df$household_compliance_rate[i] + learning,  
          df$household_fb[i] == 4 & !df$household_flood_current_year[i] ~ df$household_compliance_rate[i] + learning/r )
        
        df$household_compliance_rate[i + 1] = pmax(0, pmin(1, df$household_compliance_rate[i + 1]))
        df$household_learning_current_year[i] <- df$household_compliance_rate[i+1] - df$household_compliance_rate[i]
        df$household_learning_accumulated[i] <- sum(df$household_learning_current_year[1:(i + 1)], na.rm = TRUE)
      }
    }
    
    df$household_flood_once <- cumany(df$household_flood_current_year)
    df$household_flood_counter <- cumsum(df$household_flood_current_year)
...
# model: integrated ----
model_integrated <- function(list_development_area, n_switch, n, df_household, pfb, household_compliance_threshold, ua_2, f, learning, r) {  
  #n <- n; n_switch <- c(10,25); h <- household_initial; m <- household_growth; b <- 10 ; household_compliance_threshold <- 0.7; ua_2 <- 100 ; f <- 0.5 ; learning <- 0.3 ; r <- 7 
  #pfb <- c(0.25, 0.25, 0.25, 0.25)
  ...
  list_household_inter <- list_development_area %>%
    ...
    model_reduction_vulnerability(., household_compliance_threshold) %>% 
    model_reduction_hazard(., ua_2) %>%
    model_risk(., household_compliance_threshold, f, learning, r)
  
  household_flood_per_year <- (unlist(map_dbl(list_household_inter, ~ sum(.x$household_flood_current_year == TRUE))) / unlist(map_dbl(list_household_inter, ~ nrow(.x)))) * 100
...
# calibration ----
# parameters 
n <- n; n_switch <- c(15,25); pfb=c(0,1,0,0); household_compliance_threshold <- 0.9; ua_2 <- 100 ; f <- 0.2 ; learning <- 0.1 ; r <- 7 
...
compare_observed_simulated <- function(df_observed, list_model, prefix) {
    ...
    df <- df_observed_simulated %>%
      ...
      pivot_wider(
        ...
        values_from = c(variable, "year_current", "household_year_entry", "household_compliance_rate",
                        ...
                        "household_measure", "household_flood_current_year", "household_flood_once", "flood_depth")
      ) %>%
  ...
  plot2 <- conf_matrix(df_observed_simulated, 2024, "household_flood_current_year", "flood current year")$plot
  ...
  map2 <- conf_matrix(df_observed_simulated, 2024, "household_flood_current_year", "flood current year")$map
  ...
  df_comparison <- df_observed_simulated %>%
    ...
    summarise(
      ...
      household_flood_current_year = mean(household_flood_current_year, na.rm = TRUE) * 100,
    ...
    pivot_longer(
      cols = c(household_flood_once, household_flood_current_year, household_measure_flood_proof, household_measure),
      names_to = "variable",
      values_to = "percentage"
    ) %>%
    pivot_wider(names_from = source, values_from = percentage) %>%
    mutate(error = observed - simulated)
    ...
    filter(year_current == max(year_current))
  {
    ...
    plot_household_flood_per_year <- 
      ...
      annotate("point", 
               x = 2024, 
               y = df_comparison %>% filter(year_current == 2024, variable == "household_flood_current_year") %>% 
                 pull(observed), 
               size = 2) +
      annotate("text", 
               x = 2024, 
               y = df_comparison %>% filter(year_current == 2024, variable == "household_flood_current_year") %>% 
                 pull(observed), 
               label = paste("survey 2024", "\nerror %:", round(df_comparison$error[df_comparison$year_current == 2024 & df_comparison$variable == "household_flood_current_year"], 0)),
               vjust = -0.5) +
    ...
    plot_household_measures_flood_proof <- 
      ...
      geom_line() +
      annotate("point", x = 2017, 
               y = df_comparison %>% filter(year_current == 2017, variable == "household_measure_flood_proof") %>% 
                 pull(observed)) +
      annotate("text", x = 2017, 
               y = df_comparison %>% filter(year_current == 2017, variable == "household_measure_flood_proof") %>% 
                 pull(observed), 
               label = paste("survey 2024", "\nerror %:", round(df_comparison$error[df_comparison$year_current == 2017 & df_comparison$variable == "household_measure_flood_proof"], 0)), 
               vjust = -0.5) +
      annotate("point", 
               x = 2024, 
